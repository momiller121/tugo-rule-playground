...
internals.isTuGoEligible = (request) => {
    ...
    let { origin, destination, bookingDate, tripDate } = request.query;

    // Cast the YYYYMMDD string params to Date objects in the origin timezone
    if (!bookingDate) {
        bookingDate = utcToZonedTime(new Date(), Hoek.reach(airports[origin], 'timezone'));
    }
    else {
        bookingDate = utcToZonedTime(parse(bookingDate, 'yyyyMMdd', new Date()), Hoek.reach(airports[origin], 'timezone'));
    }

    tripDate = utcToZonedTime(parse(tripDate, 'yyyyMMdd', new Date()), Hoek.reach(airports[origin], 'timezone'));
